...
{
    if (s->reverse_bounce_active || !cfg->has_reverse_bounce_at || t < cfg->reverse_bounce_at_s)
        return;
...
static int crank_direction_at(lab_t *s, const lab_cfg_t *cfg, double t)
{
    ...
    if (s->reverse_bounce_active) return -1;
...
static void update_pas_line(lab_t *s, const lab_cfg_t *cfg, double t)
{
    if (s->reverse_bounce_active) {
        ...
        if (--s->reverse_bounce_hold_ticks == 0U) s->reverse_bounce_active = false;
        return;
    }
...
static void lab_tick(lab_t *s, const lab_cfg_t *cfg, bool emit)
{
    ...
    bool miss = cfg->miss_tick_every_n != 0U && (s->tick % cfg->miss_tick_every_n) == 0U;
    ...
    if (emit) {
        ...
        printf(
            ...
            "%u,"                           /* 28: ride_interval */
            "%.3f,"                         /* 29: torque_mean */
            "%d,"                           /* 30: crank_direction */
            "%u,%u,%lld,%u,"                /* 31-34: pas_ab, pas_normal_ab, pas_transition_index, pas_last_step */
            "%u,%u,%u,%u,%u,"               /* 35-39: pas_dir_state, pas_inhibit_reason, pas_fwd, pas_rev, pas_backpedal */
            "%u,%u,%u,%u,%u,"               /* 40-44: sampler fwd, rev, inv, glitch, overflow */
            "%u,%u,%u,%u,"                  /* 45-48: torque_cmd_valid, pas_cmd_valid, rider_tq, rider_pas */
            "%u,%u,%u,%u,%u,"               /* 49-53: out_elapsed, out_fg, fg_delay, rev_bounce, pas_inv_seq */
            "%llu\n",                       /* 54: fast_iq_slew_ticks */
            t, angle,
