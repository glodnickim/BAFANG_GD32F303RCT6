...
& powershell -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot 'build-firmware.ps1') -Target M820_BL820 -Profile $Profile -Variant normal -BuildMode Reserved -Version $pair.Versions[0] -VersionStateRoot $StateRoot -OutputDir $OutputDir
if($LASTEXITCODE -ne 0){ throw 'NORMAL failed; both pair numbers remain consumed.' }
& powershell -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot 'build-firmware.ps1') -Target M820_BL820 -Profile $Profile -Variant diagnostic -BuildMode Reserved -Version $pair.Versions[1] -VersionStateRoot $StateRoot -OutputDir $OutputDir
